...
class Runner(object):
    ...
    def run(self, temp):
        ...
        for n in range(self.nsteps):
            ...
            obs, rewards, dones, _, illegal = self.env.step_vs(actions, side)
            self.update_obs(obs)
...
def learn(policy, env, seed, nsteps, nstack=4, total_timesteps=int(80e6), vf_coef=0.5, ent_coef=0.01,
    ...
    model = Model(policy=policy, ob_space=ob_space, ac_space=ac_space, nenvs=nenvs, nsteps=np.sqrt(nsteps),
                  nstack=nstack,
                ...
                for i in range(len(batch)):
                    ...
                    if data_augmentation:
                        ...
                        pl, vl, pe = train_without_data_augmentation(obs, states, rewards,
                                                                     masks, actions, values, model, temp)
                        policy_loss_sv.append(pl)
                        value_loss_sv.append(vl)
                        policy_entropy_sv.append(pe)
